...
# --- External API Key Input (for GPUStack) ---
...
with st.sidebar:
    ...
    if user_api_key:
        st.session_state.gpustack_api_key = user_api_key
                ...
                if corpus_location:
                    ...
                    if corpus_path.exists() and corpus_path.is_dir():
                        ...
                        if valid_corpora:
                            ...
                            # Check if topics file already exists and show download button
                            ...
                            if topics_file.exists():
                                ...
                                topics_data = json.dumps(records, indent=4)
                                st.download_button(
                                    label="⬇️ Download Corpus with Topics (JSON)",
                                    data=topics_data,
                                    file_name="corpus_table.with_topics.json",
                                    mime="application/json",
                                    key="download_existing_topics"
                                )
                            ...
                            llm_backend = st.radio(
                                "Select LLM Backend",
                                ["Ollama (Local)", "GPUStack (Remote)"],
                                help="Use local resources (free, no key) or remote GPU cluster (requires API key)."
                            )

                            # Determine model and readiness based on backend
                            backend_ready = False
                            extraction_model = ""
                            
                            if llm_backend == "Ollama (Local)":
                                try:
                                    # Fetch list properly handling object vs dict response
                                    models_response = ollama.list()
                                    ollama_models = []
                                    
                                    # Handle 'models' attribute if it's an object, or key if dict
                                ...
                                # GPUStack
                                extraction_model = "gpt-oss-120b" # Default powerful model
                                if not st.session_state.gpustack_api_key:
                                    st.warning("⚠️ Please enter your GPUStack API Key in the sidebar.")
                                    backend_ready = False
                                else:
                                    backend_ready = True
