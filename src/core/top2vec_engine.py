...
def _is_reduced_model(topic_model: Top2Vec, target_topics: Union[int, str]) -> bool:
    if target_topics == "auto" or not isinstance(target_topics, int):
        return False
    return topic_model.get_num_topics(reduced=True) < topic_model.get_num_topics(reduced=False)
...
def generate_top2vec_keywords_df(topic_model: Top2Vec, target_topics: Union[int, str]) -> pd.DataFrame:
    ...
    size_map: Dict[int, int] = {int(topic_num): int(size) for size, topic_num in zip(topic_sizes, size_topic_nums)}
...
def generate_top2vec_document_topics_df(
    ...
    if len(doc_topics_flat) != len(original_df):
        raise ValueError(
            f"Top2Vec output length ({len(doc_topics_flat)}) does not match dataframe length ({len(original_df)})."
        )
